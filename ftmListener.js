...
async function checkPending() {
  fs.readFile(path.resolve(__dirname, 'ftmBlock.json'), async (err, blockData) => {
      ...
      let latest = await web3Ftm.eth.getBlockNumber();
      latest = latest - 30
      console.log(lastcheckBlock,latest)
      blockData["lastblock"] = latest;

      FTM_BRIDGE_INSTANCE.getPastEvents({},
          {
      ...
      fs.writeFile(path.resolve(__dirname, './ftmBlock.json'), JSON.stringify(blockData), (err) => {
          if (err);
          console.log(err);
      });
  });
...
async function bscBridgeBack(user,amount,id,fromTransactionHash) {
    ...
    isAlreadyProcessed = await BSC_BRIDGE_INSTANCE.methods.getBridgeStatus(id,250).call();
    ...
    var encodeABI = BSC_BRIDGE_INSTANCE.methods.swapBack(user,amount,id,250).encodeABI();
