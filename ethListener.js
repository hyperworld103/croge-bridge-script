...
async function checkPending() {
  fs.readFile(path.resolve(__dirname, 'ethBlock.json'), async (err, blockData) => {
      ...
      let latest = await web3Eth.eth.getBlockNumber();
      latest = latest - 10
      console.log(lastcheckBlock,latest)
      blockData["lastblock"] = latest;

      ETH_BRIDGE_INSTANCE.getPastEvents({},
          {
...
async function bscBridgeBack(user,amount,id,fromTransactionHash) {
    ...
    isAlreadyProcessed = await BSC_BRIDGE_INSTANCE.methods.getBridgeStatus(id,1).call();
    ...
    var encodeABI = BSC_BRIDGE_INSTANCE.methods.swapBack(user,amount,id,1).encodeABI();
